...
fn main() {
    for file in env::args().skip(1) {
        ...
        if let Ok(dst_name) = File::open(&file).and_then(read_rar_file) {
            let mut dst_path = parent.map_or(PathBuf::new(), Path::to_path_buf);
            ...
            println!("error opening RAR archive {}", file);
        }
    }
}

fn read_rar_file(mut input: File) -> Result<String> {
    let mut signature = [0; 7];
    try!(input.read(&mut signature));

    if signature != [0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x00] {
        return Err(Error::new(ErrorKind::InvalidInput,
                              "input is not an RAR archive!".to_string()));
    }
    ...
    loop {
        ...
        let mut hd = MemReader::new(&mut input, 7)?;
        ...
        match header_type {
            0x74 => {
                if let Some(dir_name) = parse_file_header(&mut input, header_flags, header_size)? {
                    dirs.push(dir_name);
                }
            }
            0x7a => {
            ...
            _ => {
                ...
                return Err(Error::new(ErrorKind::InvalidInput,
                                      format!("error: unknown header type at 0x{:x}", pos)));
            }
        }
    }
    ...
    if dirs.len() > 1 {
        return Err(Error::new(ErrorKind::InvalidInput,
                              "error: RAR archive contains more than two directories."
                                  .to_string()));
    } else {
        return dirs.pop().ok_or(Error::new(ErrorKind::InvalidInput,
                                           "error: RAR archive contains no directories."
                                               .to_string()));
    }
}

fn parse_other_header(input: &mut Seek, size: u16) -> Result<()> {
    input.seek(SeekFrom::Current((size as i64) - 7))?;
    return Ok(());
}

fn parse_service_header(input: &mut File, header_size: u16) -> Result<()> {
    let mut hd = MemReader::new(input, (header_size as usize) - 7)?;
    let skip_size = hd.get4() as i64;
    input.seek(SeekFrom::Current(skip_size))?;
    return Ok(());
}
